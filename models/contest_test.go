...
func TestContest_GetListCount(t *testing.T) {
	type fields struct {
		Model            Model
		ContestName      string
		ContestDesc      string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := c.GetListCount(tt.args.status)
			if (err != nil) != tt.wantErr {
				t.Errorf("GetListCount() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if got != tt.want {
				t.Errorf("GetListCount() got = %v, want %v", got, tt.want)
			}
		})
	}
}

func TestContest_Insert1(t *testing.T) {
	type fields struct {
		Model            Model
		ContestName      string
		ContestDesc      string
